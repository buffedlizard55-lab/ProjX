#!/usr/bin/env python3
"""Session 32 — Verification Apply Pipeline (Japanese-Wikipedia slice of the Wikidata model pool, page 3, OFFSET 300; adapted from session31_apply.py).

Reads the line-by-line verification verdicts from
data/research/verification_s32_log.tsv (pipe-delimited:
    qid|name|dob|verdict|category|evidence|source_url)
and joins them with data/research/s32_verify_buckets.json (handles incl. optional second Instagram handle, wiki).

- PROMOTE            -> data/catalog.json entries (catalogType=social, ID from W-2026-2443)
- REVIEW             -> data/catalog.json reviewQueue (ID from R-2026-422)
...
Usage:
  python3 scripts/session31_apply.py [--dry-run]
"""
...
LOG_PATH = ROOT / "data" / "research" / "verification_s32_log.tsv"
BUCKETS_PATH = ROOT / "data" / "research" / "s32_verify_buckets.json"
...
def evidence_flags(source_url: str, dob: date, evidence: str = "") -> list[str]:
    ...
    if evidence and "name alias noted" in evidence:
        # article title / stage name differs from the Wikidata English label
        flags.append("NAME_ALIAS_IN_WIKIPEDIA")
    if evidence and "country of citizenship not recorded" in evidence:
        flags.append("COUNTRY_NOT_RECORDED")
    if evidence and "current-use status unverified" in evidence:
        # two Instagram handles documented in Wikidata for one person
        flags.append("MULTIPLE_IG_HANDLES_DOCUMENTED")
    return flags
...
def assign_categories(cat_label: str) -> list[str]:
    ...
    if "fitness" in text or "yoga" in text:
        cats.extend(["Fitness", "Fitness Model"] if "fitness" in text else ["Fitness", "Wellness"])
    if "gravure" in text or "swimwear" in text:
        # gravure idol = Japanese swimsuit / magazine model (objective occupational category)
        cats.extend(["Modeling", "Swimwear", "Fashion"])
    if "race queen" in text:
...
def main() -> None:
    ...
    for v in verdicts:
        ...
        if ig and ig.lower() in existing_igs:
            ...
            continue
        if ig2 and ig2.lower() in existing_igs:
            skipped.append(f"{qid} {name}: second IG already in catalog")
            continue
        if (ig and ig.lower() in seen_in_batch_i) or (ig2 and ig2.lower() in seen_in_batch_i) or (tt and tt.lower() in seen_in_batch_t):
            skipped.append(f"{qid} {name}: handle dupe in batch")
            continue
        ...
        if verdict == "PROMOTE":
            ...
            new_entries.append(_catalog_record(entry_id, name, dob, age, qid,
                                               ...
                                               v["source_url"], ig, tt, wiki, ig2))
            seen_in_batch_q.add(qid); seen_in_batch_n.add(nname)
            if ig:
                seen_in_batch_i.add(ig.lower())
            if ig2:
                seen_in_batch_i.add(ig2.lower())
            if tt:
    ...
    meta["summary"] += (
        f" Session 32 added {len(new_entries)} verified adult female creator profiles "
        f"(Japanese fashion / gravure / race-queen models, tarento and social-media creators from the "
        f"Japanese-Wikipedia slice of the Wikidata model pool, page 3) and {len(new_reviews)} REVIEW_REQUIRED candidates "
        f"with provenance from line-by-line official/trusted-source verification "
        f"(data/research/verification_s32_log.tsv, {len(verdicts)} verdicts). "
    ...
    catalog["irregularities"].append({
        "id": "IRR-2026-09-06-034",
        "type": "BATCH_INGESTION_SESSION_32",
        "entityId": None,
        "description": (
            f"Session 32 batch ingestion (WDQS female model / fitness-model pool born 1985-2008 with an Instagram or TikTok handle "
            f"and a Japanese Wikipedia article but no English article, page 3 ORDER BY item OFFSET 300: 150 rows, 150 unique, 140 new after dedup): "
            f"{len(verdicts)} line-by-line verification verdicts "
            ...
            f"(scope-ambiguous actresses / announcers / idols / tarento / a politician / a newscaster and several women whose modeling is documented only as former activity, "
            f"three DOB_CONFLICT pages whose lead birth year differs from Wikidata, three AGE_PARTIAL pages with no or partial birth date); "
            f"{len([s for s in skipped if 'REJECT' in s])} rejected. "
            f"Youngest promoted entry is 19 (born 2006-09-20; adult on the check date). One collision with a review-queue candidate "
            f"(Risa Watanabe R-2026-069, a different woman) was disambiguated with katakana in the display name. "
            f"See data/research/verification_s32_log.tsv."
        ),
...
def _catalog_record(entry_id: str, name: str, dob: date, age: int, qid: str,
                    ...
                    ig: str, tt: str, wiki: str, ig2: str = "") -> dict:
    ...
    social_accounts = []
    for handle, second in ((ig, False), (ig2, True)):
        if not handle:
            continue
        ig_clean = clean_handle("Instagram", handle)
        ig_url = make_profile_url("Instagram", handle)
        note = (f"Handle documented on official / public profile for {name}. No public count observed."
                if not second else
                f"Second Instagram handle documented in Wikidata (P2003) for {name}; current-use status "
                f"unverified. No public count observed.")
    ...
    notes = f"Session 32 verified (WDQS model-pool harvest, Japanese-Wikipedia slice page 3; line-by-line ja-wiki lead-sentence check). Category: {cat_label}. Evidence: {evidence}"
    return {
        "id": entry_id,
        "displayName": name,
...
def _review_record(rnum: int, qid: str, name: str, ig: str, tt: str,
    ...
    return {
        ...
        "discoveryCategory": "Session 32 (modeling / gravure / race queen / creator — WDQS ja-wiki harvest, page 3)",
        "evidenceFound": {
            "summary": notes[:400],
        ...
        "notes": f"Session 32 verification (2026-09-06), Wikidata Q-ID {qid}. {notes}",
        "followerCountDisplay": UNKNOWN_COUNT,
        "followerCountNumeric": None,
