#!/usr/bin/env python3
"""Session 28 — Verification Apply Pipeline (model/pageant/creator WDQS harvest page 1, adapted from session27_apply.py).

Reads the line-by-line verification verdicts from
data/research/verification_s28_log.tsv (pipe-delimited:
    qid|name|dob|verdict|category|evidence|source_url)
and joins them with data/research/s28_verify_buckets.json (handles, wiki).

- PROMOTE            -> data/catalog.json entries (catalogType=social, ID from W-2026-2046)
- REVIEW             -> data/catalog.json reviewQueue (ID from R-2026-260)
...
Usage:
  python3 scripts/session28_apply.py [--dry-run]
"""
...
LOG_PATH = ROOT / "data" / "research" / "verification_s28_log.tsv"
BUCKETS_PATH = ROOT / "data" / "research" / "s28_verify_buckets.json"
...
def main() -> None:
    ...
    meta["summary"] += (
        f" Session 28 added {len(new_entries)} verified adult female creator profiles "
        f"(models, beauty-pageant titleholders, fitness/TV/social-media creators from the "
        f"Wikidata model-pool page beyond Q123694020) and {len(new_reviews)} REVIEW_REQUIRED candidates "
        f"with provenance from line-by-line official/trusted-source verification "
        f"(data/research/verification_s28_log.tsv, {len(verdicts)} verdicts). "
    ...
    catalog["irregularities"].append({
        "id": "IRR-2026-09-06-030",
        "type": "BATCH_INGESTION_SESSION_28",
        "entityId": None,
        "description": (
            f"Session 28 batch ingestion (WDQS female model / fitness-model / pageant / creator pool born 1985-2008 with an Instagram or TikTok handle and an English Wikipedia article, page 1 beyond Q123694020: 150 rows, 147 unique, 143 new after dedup): {len(verdicts)} line-by-line verification verdicts "
            ...
            f"AGE_PARTIAL pages without a birth date, DOB_CONFLICT between en-wiki and Wikidata, "
            f"one WIKI_REDIRECT, one MINOR_UNDERAGE hold, and one candidate held because the en-wiki lead "
            f"reports an arrest pending trial); {len([s for s in skipped if 'REJECT' in s])} "
            f"rejected (deceased). See data/research/verification_s28_log.tsv."
        ),
...
def _catalog_record(entry_id: str, name: str, dob: date, age: int, qid: str,
    ...
    notes = f"Session 28 verified (WDQS model/pageant/creator harvest, line-by-line English Wikipedia lead/infobox check). Category: {cat_label}. Evidence: {evidence}"
    return {
        "id": entry_id,
        "displayName": name,
...
def _review_record(rnum: int, qid: str, name: str, ig: str, tt: str,
    ...
    return {
        ...
        "discoveryCategory": "Session 28 (modeling / beauty pageant / fitness / creator — WDQS harvest)",
        "evidenceFound": {
            "summary": notes[:400],
        ...
        "notes": f"Session 28 verification (2026-09-06), Wikidata Q-ID {qid}. {notes}",
        "followerCountDisplay": UNKNOWN_COUNT,
        "followerCountNumeric": None,
