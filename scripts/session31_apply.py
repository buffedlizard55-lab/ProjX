#!/usr/bin/env python3
"""Session 31 — Verification Apply Pipeline (Japanese-Wikipedia slice of the Wikidata model pool, page 2, OFFSET 150; adapted from session30_apply.py).

Reads the line-by-line verification verdicts from
data/research/verification_s31_log.tsv (pipe-delimited:
    qid|name|dob|verdict|category|evidence|source_url)
and joins them with data/research/s31_verify_buckets.json (handles incl. optional second Instagram handle, wiki).

- PROMOTE            -> data/catalog.json entries (catalogType=social, ID from W-2026-2316)
- REVIEW             -> data/catalog.json reviewQueue (ID from R-2026-404)
...
LOG_PATH = ROOT / "data" / "research" / "verification_s31_log.tsv"
BUCKETS_PATH = ROOT / "data" / "research" / "s31_verify_buckets.json"
...
def main() -> None:
    ...
    meta["summary"] += (
        f" Session 31 added {len(new_entries)} verified adult female creator profiles "
        f"(Japanese fashion / gravure / race-queen models, tarento and social-media creators from the "
        f"Japanese-Wikipedia slice of the Wikidata model pool, page 2) and {len(new_reviews)} REVIEW_REQUIRED candidates "
        f"with provenance from line-by-line official/trusted-source verification "
        f"(data/research/verification_s31_log.tsv, {len(verdicts)} verdicts). "
    ...
    catalog["irregularities"].append({
        "id": "IRR-2026-09-06-033",
        "type": "BATCH_INGESTION_SESSION_31",
        "entityId": None,
        "description": (
            f"Session 31 batch ingestion (WDQS female model / fitness-model pool born 1985-2008 with an Instagram or TikTok handle "
            f"and a Japanese Wikipedia article but no English article, page 2 ORDER BY item OFFSET 150: 150 rows, 150 unique, 145 new after dedup): "
            ...
            f"(scope-ambiguous actresses / announcers / idols / a businesswoman / a beauty specialist with no documented modeling or creator activity, "
            f"one adult-video performer held for a manual scope decision, two AGE_PARTIAL gravure/model pages that give no birth date); "
            f"{len([s for s in skipped if 'REJECT' in s])} rejected. "
            f"Youngest promoted entries are 21 (born 2005). Two same-romanisation names in the batch (Natsumi Saito x2) and one collision with an "
            f"existing entry (Yui Koike W-2026-1900, a different woman) were disambiguated with kanji in the display name. "
            f"See data/research/verification_s31_log.tsv."
        ),
...
def _catalog_record(entry_id: str, name: str, dob: date, age: int, qid: str,
    ...
    notes = f"Session 31 verified (WDQS model-pool harvest, Japanese-Wikipedia slice page 2; line-by-line ja-wiki lead-sentence check). Category: {cat_label}. Evidence: {evidence}"
    return {
        "id": entry_id,
        "displayName": name,
...
def _review_record(rnum: int, qid: str, name: str, ig: str, tt: str,
    ...
    return {
        ...
        "discoveryCategory": "Session 31 (modeling / gravure / race queen / creator — WDQS ja-wiki harvest, page 2)",
        "evidenceFound": {
            "summary": notes[:400],
        ...
        "notes": f"Session 31 verification (2026-09-06), Wikidata Q-ID {qid}. {notes}",
        "followerCountDisplay": UNKNOWN_COUNT,
        "followerCountNumeric": None,
