#!/usr/bin/env python3
"""Session 26 — Verification Apply Pipeline.

Reads the line-by-line verification verdicts from
data/research/verification_s26_log.tsv (pipe-delimited:
    qid|name|dob|verdict|category|evidence|source_url)
and joins them with data/research/s26_verify_buckets.json (handles, occ, wiki).
...
LOG_PATH = ROOT / "data" / "research" / "verification_s26_log.tsv"
BUCKETS_PATH = ROOT / "data" / "research" / "s26_verify_buckets.json"
...
def load_buckets() -> dict:
    ...
    for item in b.get("wiki", []) + b.get("agency", []):
...
def main() -> None:
    ...
    meta["summary"] += (
        f" Session 26 added {len(new_entries)} verified adult female creator profiles "
        f"(NCAA/European/international volleyball, beach volleyball, swimwear/bikini fashion, "
        f"fitness, modeling, dance/TV) and {len(new_reviews)} REVIEW_REQUIRED candidates "
        f"with provenance from line-by-line official/trusted-source verification "
        f"(data/research/verification_s26_log.tsv, {len(verdicts)} verdicts). "
    ...
    catalog["irregularities"].append({
        "id": "IRR-2026-09-06-028",
        "type": "BATCH_INGESTION_SESSION_26",
        "entityId": None,
        "description": (
            f"Session 26 batch ingestion: {len(verdicts)} line-by-line verification verdicts "
            ...
            f"(including 1 minor, Vittoria Seixas Q122866537, DOB 2008-12-22, "
            f"re-evaluate after 2027-12-22); {len([s for s in skipped if 'REJECT' in s])} "
            f"rejected (deceased/unverifiable). See data/research/verification_s26_log.tsv."
        ),
...
def _catalog_record(entry_id: str, name: str, dob: date, age: int, qid: str,
    ...
    notes = f"Session 26 verified. Category: {cat_label}. Evidence: {evidence}"
    return {
        "id": entry_id,
        "displayName": name,
...
def _review_record(rnum: int, qid: str, name: str, ig: str, tt: str,
    ...
    return {
        ...
        "discoveryCategory": "Session 26 (volleyball / beach volleyball / swimwear / fitness / modeling)",
        "evidenceFound": {
            "summary": notes[:400],
        ...
        "notes": f"Session 26 verification (2026-09-06), Wikidata Q-ID {qid}. {notes}",
        "followerCountDisplay": UNKNOWN_COUNT,
        "followerCountNumeric": None,
