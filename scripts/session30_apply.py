#!/usr/bin/env python3
"""Session 30 — Verification Apply Pipeline (Japanese-Wikipedia slice of the Wikidata model pool, page 1; adapted from session29_apply.py).

Reads the line-by-line verification verdicts from
data/research/verification_s30_log.tsv (pipe-delimited:
    qid|name|dob|verdict|category|evidence|source_url)
and joins them with data/research/s30_verify_buckets.json (handles incl. optional second Instagram handle, wiki).

- PROMOTE            -> data/catalog.json entries (catalogType=social, ID from W-2026-2206)
- REVIEW             -> data/catalog.json reviewQueue (ID from R-2026-380)
...
Usage:
  python3 scripts/session30_apply.py [--dry-run]
"""
...
LOG_PATH = ROOT / "data" / "research" / "verification_s30_log.tsv"
BUCKETS_PATH = ROOT / "data" / "research" / "s30_verify_buckets.json"
...
def main() -> None:
    ...
    meta["summary"] += (
        f" Session 30 added {len(new_entries)} verified adult female creator profiles "
        f"(Japanese fashion / gravure / race-queen models, tarento and social-media creators from the "
        f"Japanese-Wikipedia slice of the Wikidata model pool, page 1) and {len(new_reviews)} REVIEW_REQUIRED candidates "
        f"with provenance from line-by-line official/trusted-source verification "
        f"(data/research/verification_s30_log.tsv, {len(verdicts)} verdicts). "
    ...
    catalog["irregularities"].append({
        "id": "IRR-2026-09-06-032",
        "type": "BATCH_INGESTION_SESSION_30",
        "entityId": None,
        "description": (
            f"Session 30 batch ingestion (WDQS female model / fitness-model pool born 1985-2008 with an Instagram or TikTok handle "
            f"and a Japanese Wikipedia article but no English article, page 1 ORDER BY item: 150 rows, 148 unique, 135 new after dedup): "
            ...
            f"(scope-ambiguous idols / singers / actresses / announcers with no documented modeling or creator activity, "
            f"three adult-video performers held for a manual scope decision, AGE_PARTIAL pages that give no full birth date, "
            f"DOB_CONFLICT between the ja-wiki lead and Wikidata); "
            f"{len([s for s in skipped if 'REJECT' in s])} rejected (deceased YouTuber). "
            f"Five promoted models turned 18 earlier in 2026 (18th birthdays 2026-01-02 .. 2026-07-01, all before the check date). "
            f"Two entries carry two Wikidata-documented Instagram handles each (flag MULTIPLE_IG_HANDLES_DOCUMENTED). "
            f"See data/research/verification_s30_log.tsv."
        ),
...
def _catalog_record(entry_id: str, name: str, dob: date, age: int, qid: str,
    ...
    notes = f"Session 30 verified (WDQS model-pool harvest, Japanese-Wikipedia slice; line-by-line ja-wiki lead-sentence check). Category: {cat_label}. Evidence: {evidence}"
    return {
        "id": entry_id,
        "displayName": name,
...
def _review_record(rnum: int, qid: str, name: str, ig: str, tt: str,
    ...
    return {
        ...
        "discoveryCategory": "Session 30 (modeling / gravure / race queen / creator — WDQS ja-wiki harvest)",
        "evidenceFound": {
            "summary": notes[:400],
        ...
        "notes": f"Session 30 verification (2026-09-06), Wikidata Q-ID {qid}. {notes}",
        "followerCountDisplay": UNKNOWN_COUNT,
        "followerCountNumeric": None,
