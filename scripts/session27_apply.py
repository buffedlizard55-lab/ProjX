#!/usr/bin/env python3
"""Session 27 — Verification Apply Pipeline (volleyball WDQS harvest, adapted from session26_apply.py).

Reads the line-by-line verification verdicts from
data/research/verification_s27_log.tsv (pipe-delimited:
    qid|name|dob|verdict|category|evidence|source_url)
and joins them with data/research/s27_verify_buckets.json (handles, wiki).

- PROMOTE            -> data/catalog.json entries (catalogType=social, ID from W-2026-1861)
- REVIEW             -> data/catalog.json reviewQueue (ID from R-2026-155)
- MINOR_UNDERAGE     -> reviewQueue with MINOR_UNDERAGE flag (re-evaluate after 2027-12-22)
...
Usage:
  python3 scripts/session26_apply.py [--dry-run]
"""
...
LOG_PATH = ROOT / "data" / "research" / "verification_s27_log.tsv"
BUCKETS_PATH = ROOT / "data" / "research" / "s27_verify_buckets.json"
...
def evidence_flags(source_url: str, dob: date) -> list[str]:
...
def load_buckets() -> dict:
    ...
    for item in b.get("volleyball", []) + b.get("wiki", []) + b.get("agency", []):
...
def main() -> None:
    ...
    for v in verdicts:
        ...
        if verdict == "PROMOTE":
            ...
            note = v["evidence"]
            if verdict == "MINOR_UNDERAGE":
                note = (f"Underage: DOB {v['dob']} (age {age_on(parse_dob(v['dob']))} as of "
                        f"{TODAY.isoformat()}). Keep in queue; re-evaluate from 2027-12-22. "
                        f"Do NOT discard. {note}")
            max_r += 1
            new_reviews.append(_review_record(max_r, qid, name, ig, tt,
    ...
    meta["summary"] += (
        f" Session 27 added {len(new_entries)} verified adult female creator profiles "
        f"(Japanese V.LEAGUE, German Bundesliga, Russian Superliga, Thai, Slovak, Dutch, Korean, "
        f"Italian and NCAA/US-pro volleyball, beach volleyball) and {len(new_reviews)} REVIEW_REQUIRED candidates "
        f"with provenance from line-by-line official/trusted-source verification "
        f"(data/research/verification_s27_log.tsv, {len(verdicts)} verdicts). "
    ...
    catalog["irregularities"].append({
        "id": "IRR-2026-09-06-029",
        "type": "BATCH_INGESTION_SESSION_27",
        "entityId": None,
        "description": (
            f"Session 27 batch ingestion (WDQS female volleyball / beach-volleyball players born 1985-2008 with an Instagram or TikTok handle and a Wikipedia article, 743-row population, 45 new after dedup): {len(verdicts)} line-by-line verification verdicts "
            ...
            f"(scope-ambiguous: a comedian, a singer and a bobsledder surfaced by the "
            f"sport=volleyball statement); {len([s for s in skipped if 'REJECT' in s])} "
            f"rejected. See data/research/verification_s27_log.tsv."
        ),
...
def _catalog_record(entry_id: str, name: str, dob: date, age: int, qid: str,
    ...
    notes = f"Session 27 verified (WDQS volleyball harvest, line-by-line Wikipedia lead-sentence check). Category: {cat_label}. Evidence: {evidence}"
    return {
        "id": entry_id,
        "displayName": name,
        ...
        "flags": evidence_flags(source_url, dob),
        "notes": notes,
...
def _review_record(rnum: int, qid: str, name: str, ig: str, tt: str,
    ...
    return {
        ...
        "discoveryCategory": "Session 27 (women's volleyball / beach volleyball — WDQS harvest)",
        "evidenceFound": {
            "summary": notes[:400],
        ...
        "notes": f"Session 27 verification (2026-09-06), Wikidata Q-ID {qid}. {notes}",
        "followerCountDisplay": UNKNOWN_COUNT,
        "followerCountNumeric": None,
